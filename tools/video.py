...
def assemble_video(image_paths: List[str], audio_path: str, subtitle_path: str, output_path: str, duration_sec: float) -> None:
    ...
    try:
        ...
        img_duration = duration_sec / num_images
        width, height = 1080, 1920
        fps = 30
 
        # Ruta absoluta requerida por el filtro subtitles de FFmpeg
        ...
        if len(inputs) == 1:
            ...
            for i in range(1, len(inputs)):
                offset = i * img_duration - 0.5
                if offset < 0:
                    offset = 0.1
                video = ffmpeg.filter([video, inputs[i]], 'xfade', transition='fade', duration=0.5, offset=offset)
        ...
        out = ffmpeg.output(video, audio, output_path, vcodec='libx264', acodec='aac', shortest=None, pix_fmt='yuv420p')
